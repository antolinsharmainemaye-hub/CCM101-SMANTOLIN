...
The `-e` flags were used to set environment variables inside the Docker container. In this deployment, `MINIO_ROOT_USER` set the administrator username to `cloudadmin`, while `MINIO_ROOT_PASSWORD` set the administrator password used to log in to the MinIO Web Console.
